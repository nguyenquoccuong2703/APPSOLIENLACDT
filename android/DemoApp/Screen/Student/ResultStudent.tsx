...
const ResultStudent: React.FC = () => {
    ...
    const fetchScoresAndDetails = async () => {
        try {
            const [scoresResponse, classesResponse, teachersResponse, subjectsResponse] = await Promise.all([
                fetch(`http://192.168.137.1:3000/Score?StudentId=${selectedStudent?.StudentId}`),
